...
static bool _registrationCompleted = false;
static PROV_DEVICE_RESULT _registrationStatus = PROV_DEVICE_RESULT_OK;
static char *_iothub_uri = NULL;
...
// Model ID
static char *modelIdPayload = NULL;
static const char g_ModelId[] = "dtmi:com:example:azuresphere:sampledevice;1"; // "urn:testazuresphere:AzureSphereSampleDevice_614:1";
static const char g_dps_PayloadFormatForModelId[] = "{\"modelId\":";
...
void registerDeviceCallback(PROV_DEVICE_RESULT register_result, const char *iothub_uri,
                            const char *device_id, void *user_context)
{
    _registrationCompleted = true;
    _registrationStatus = register_result;
    if (register_result == PROV_DEVICE_RESULT_OK) {
        if (iothub_uri != NULL) {
            _iothub_uri = calloc(strlen(iothub_uri) + 1, sizeof(char));
            strncpy(_iothub_uri, iothub_uri, strlen(iothub_uri));
        }
    }
}
...
static bool SetUpAzureIoTHubClientWithDps(void)
{
    ...
    _registrationStatus = PROV_DEVICE_RESULT_OK;
    bool networkingReady = false;
    modelIdPayload = calloc(strlen(g_dps_PayloadFormatForModelId) + strlen(g_ModelId) + 4, sizeof(char));

    strcat(modelIdPayload, g_dps_PayloadFormatForModelId);
    strcat(modelIdPayload, "\"");
    strcat(modelIdPayload, g_ModelId);
    strcat(modelIdPayload, "\"}");
    ...
    if (!networkingReady) {
        Log_Debug("Error: Networking not Ready");
        return false;
    }
    ...
    PROV_DEVICE_LL_HANDLE prov_handle =
        Prov_Device_LL_Create(DPS_URL, scopeId, Prov_Device_MQTT_Protocol);
    if (prov_handle == NULL) {
        ret = false;
        goto cleanup;
    }
    ...
    int device_id = 0;
    PROV_DEVICE_RESULT result = Prov_Device_LL_SetOption(prov_handle, "SetDeviceId", &device_id);
    if (result != PROV_DEVICE_RESULT_OK) {
        ret = false;
        goto cleanup;
    }

    result = Prov_Device_LL_Set_Provisioning_Payload(prov_handle, modelIdPayload);
    if (result != PROV_DEVICE_RESULT_OK) {
        Log_Debug("Error: Failed setting provisioning data");
        result = false;
    }

    result = Prov_Device_LL_Register_Device(prov_handle, registerDeviceCallback, NULL, NULL, NULL);
    if (result != PROV_DEVICE_RESULT_OK) {
        ret = false;
        goto cleanup;
    }

    static const long timeoutMs = 10000;
    static const long workDelayMs = 25;

    const struct timespec sleepTime = {.tv_sec = 0, .tv_nsec = workDelayMs * 1000 * 1000};
    long time_elapsed = 0;
    while (!_registrationCompleted && time_elapsed < timeoutMs) {
    ...
    if (_registrationCompleted) {
        if (_registrationStatus != PROV_DEVICE_RESULT_OK) {
            ret = false;
            goto cleanup;
        }

        iothubClientHandle =
            IoTHubDeviceClient_LL_CreateWithAzureSphereFromDeviceAuth(_iothub_uri, &MQTT_Protocol);
    ...
    //BL test - set ModelId to iothub handler
    if (IOTHUB_CLIENT_OK != IoTHubDeviceClient_LL_SetOption(iothubClientHandle,
            OPTION_MODEL_ID, g_ModelId)) {
        Log_Debug("Error: Set ModelId to iothub handle failed.");
        //need to stop the process? or keep it going
    }
    //BL test end

    // Enable hardware crypto
    int deviceId = 1;
    if ((result = IoTHubDeviceClient_LL_SetOption(iothubClientHandle, "SetDeviceId", &deviceId)) !=
        IOTHUB_CLIENT_OK) {
        IoTHubDeviceClient_LL_Destroy(iothubClientHandle);
                                ...
                                void *userContextCallback)
{
    ...
    if (strcmp("Command", methodName) == 0) {
                               ...
                               size_t payloadSize, void *userContextCallback)
{
    ...
    int statusLedValue = json_object_dotget_number(desiredProperties, "StatusLED");
    if (statusLedValue != -1) {
        statusLedOn = statusLedValue == 1;
        GPIO_SetValue(deviceTwinStatusLedGpioFd, statusLedOn ? GPIO_Value_Low : GPIO_Value_High);
    }

    char targetLedResponseProperty[256];
    snprintf(targetLedResponseProperty, sizeof(targetLedResponseProperty),
             "{\"StatusLED\":{\"value\":%d,\"ac\":%d,\"av\":%d,\"ad\":\"%s\"}}", statusLedOn,
             200, 3, "Successfully updated target StatusLED");

    // Report current status LED state
    if (statusLedOn) {
        TwinReportState(targetLedResponseProperty); //("{\"StatusLED\":1}");
    } else {
        TwinReportState(targetLedResponseProperty); //("{\"StatusLED\":0}");
    }
